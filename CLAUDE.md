...
| `session_group.py` | Session group management: organise sessions into experiment groups (e.g. "Strategy A vs B"). SessionGroupManager with JSON persistence (`session_groups.json`), group CRUD (create/delete/add/remove/rename), aggregate stats (avg score/steps/cost, parameter comparison). CLI: create/delete/add/remove/list/show |
...
- **Session groups**: `session_group.py` — `SessionGroupManager` class organises sessions into named experiment groups. Persists in `{log_dir}/session_groups.json`. `SessionGroup` dataclass (name/description/members). CRUD: `create_group()`, `delete_group()`, `get_group()`, `list_groups()`, `add_sessions()`, `remove_sessions()`, `rename_group()`. `group_stats()` computes aggregate stats via `SessionScorer`: avg score/steps/cost, per-parameter comparison (mean/min/max/std across members), member details. `to_dict()` / `to_markdown()` output. CLI: `python session_group.py create|delete|add|remove|list|show [--log-dir] [--format json/markdown]`. Dashboard: `/groups` page with group list + create form + group detail (stats table + parameter comparison + member list + add/remove forms), `/api/groups` GET (list) + POST (create/add/remove/delete), `/api/groups/{name}` GET (stats)

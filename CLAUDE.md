...
| `sample_data/generate_survival_horror_sample.py` | Survival horror synthetic data generator (RE-style: HP/ammo/herbs/ink_ribbons/room_danger) |
| `sample_data/generate_fighting_sample.py` | Fighting game synthetic data generator (Tekken-style: p1_hp/p2_hp/round/timer/combo_hits) |
...
| `action_recommender.py` | Action recommendation engine: learns action→parameter impacts from historical sessions, recommends top actions weighted by parameter urgency. ActionRecommender class with impact_model, recommend(), to_dict(), to_markdown(). CLI: recommend |
...
| `session_group.py` | Session group management: organise sessions into experiment groups (e.g. "Strategy A vs B"). SessionGroupManager with JSON persistence (`session_groups.json`), group CRUD (create/delete/add/remove/rename), aggregate stats (avg score/steps/cost, parameter comparison), group comparison with `compare_groups(a, b)` returning per-parameter mean diff + t-test significance + summary. Comparison charts: `plot_group_bar_comparison()` (side-by-side bar chart) and `plot_group_boxplot()` (per-parameter box plots). CLI: create/delete/add/remove/list/show/compare |
| `session_benchmark.py` | Session benchmark criteria management: per-game benchmark definitions (min steps, min score, parameter thresholds with aggregators). SessionBenchmarkManager with JSON persistence (`session_benchmarks.json`), CRUD + evaluate(session) returning pass/fail/partial BenchmarkResult. CLI: create/delete/list/show/evaluate/evaluate-all |
| `session_ab_test.py` | Formal A/B test framework: statistical comparison of two session groups with Welch's t-test, Mann-Whitney U, Cohen's d effect size, confidence intervals, power analysis. `ABTestRunner` class with `run(group_a, group_b)` → `ABTestResult` (score_test + param_tests). `StatTestResult` dataclass per metric. Helper functions: `_cohens_d()`, `_effect_size_label()`, `_confidence_interval()`, `_power_analysis()`, `run_stat_test()`. Markdown/JSON report output. CLI: run |
| `session_lifecycle.py` | Session lifecycle management: configurable retention policies (keep latest N, keep tagged, keep benchmark-passing). `SessionLifecycleManager` class with `evaluate_retention()` → keep/archive decisions, `archive()` → move sessions to archive/ subdirectory, `disk_usage()` → log directory usage report. `RetentionPolicy` dataclass with JSON persistence + `from_json()`. `RetentionDecision`/`DiskUsageReport` dataclasses. Markdown/JSON output. CLI: evaluate/archive/usage |
| `replay_annotator.py` | Replay annotation system: add free-text annotations at specific steps in session replay timelines. `ReplayAnnotator` class with `add_annotation()`, `remove_annotation()`, `get_annotations()`, `get_step_annotations()`, `list_sessions()`, `get_annotation_by_id()`. `Annotation` dataclass (id/csv_filename/step/text/category/created_at). Categories: info/warning/success/danger/milestone. Persisted in `session_annotations.json`. `to_dict()` / `to_markdown()` output. CLI: add/remove/list/show |
| `session_goal.py` | Session goal tracker: per-game objectives with param conditions + min steps. `SessionGoalManager` with JSON persistence (`session_goals.json`), CRUD (create/delete/get/list), `evaluate(session)` → `list[GoalResult]` with achieved/details, `evaluate_all()` batch evaluation, `progress_over_time()` → per-goal achievement rates across sessions. `Goal` dataclass (name/game_id/param_conditions/min_steps/description). `GoalResult` dataclass (goal_name/csv_filename/achieved/details). Reuses `ParamThreshold` from session_benchmark.py. `to_dict()` / `to_markdown()` / `results_to_markdown()` / `progress_to_markdown()` output. CLI: create/delete/list/evaluate/evaluate-all/progress |
| `strategy_history.py` | Strategy evolution tracker: cross-session strategy analysis. `StrategyHistory` class with `strategy_usage_distribution()` (steps per strategy), `strategy_effectiveness_trend()` (per-strategy param deltas over time), `strategy_transition_matrix()` (A→B switch frequency DataFrame), `optimal_strategy_for_state()` (param→best strategy mapping). Extracts data from `.session.json` strategy_switches/strategy.history. `to_dict()` / `to_markdown()` output. CLI: analyze/usage/transitions |
| `dashboard.py` | FastAPI web dashboard: browse sessions, charts, GDD docs, JSON API, action replay + action analysis pages, live monitoring (/monitor + /api/monitor), cross-analysis + anomaly alerts, session ZIP export, strategy optimizer (/optimize + /api/optimize), memory watcher (/watcher + /api/watcher/alerts + /api/watcher/events SSE), session diff (/session/diff + /api/session/diff), session tags (/api/session/{name}/tags GET+POST, tag filter on home page), session notes (/api/session/{name}/notes GET+POST), parameter predictions (/session/{name}/predict + /api/session/{name}/predict), alert notifier (/notifier + /api/notifier/config GET+POST + /api/notifier/test), session comparison overlay charts (/compare/result with per-parameter color-coded overlay plots + stats tables), session search (home page search form + /api/sessions/search JSON API), action heatmap chart on /session/{name}/actions page, session scoring (Score column on home page + /api/sessions/ranking JSON API), batch reports (/reports page + POST /api/reports/generate with Markdown/JSON/HTML output), parameter dashboard (/parameters page with correlation heatmap + distribution histograms + trend lines + lag correlations, /api/parameters/correlations JSON API), session timeline chart (integrated action bars + parameter lines + event markers on /session/{name}, /api/session/{name}/timeline JSON API), action recommendations (/session/{name}/recommend page + /api/session/{name}/recommend JSON API), session groups (/groups page with group CRUD + stats + parameter comparison, /api/groups GET+POST, /api/groups/{name} JSON API), group comparison (/groups/compare page with t-test significance table + bar chart + box plots, /api/groups/compare JSON API), session benchmarks (/benchmarks page with benchmark CRUD + evaluation results + pass/fail/partial status, /api/benchmarks GET+POST, /api/benchmarks/{game_id} JSON API), A/B test (/ab-test page with group selector + score/param comparison tables + effect size + CI + power, /api/ab-test JSON API), session lifecycle (/lifecycle page with retention policy config + keep/archive decisions + disk usage report + dry-run archive + confirm archive, /api/lifecycle GET + /api/lifecycle/archive POST), replay annotations (/session/{name}/annotations page with timeline markers + annotation form + category-colored table, /api/session/{name}/annotations GET+POST JSON API), session goals (/goals page with goal CRUD + game-specific evaluation table + achievement progress visualization, /api/goals GET+POST JSON API), dashboard overview (home page overview cards with total sessions + avg score recent/all + top strategy + anomaly count + benchmark pass rate + disk usage + SVG sparkline score trend, /api/overview JSON API), strategy history (/strategy-history page with usage pie chart + effectiveness trend line chart + transition heatmap + optimal strategy table, /api/strategy-history JSON API) |
| `demo_run.py` | E2E demo: --genre rpg/action/themepark/survival_horror/fighting → analysis → GDD → charts (no API key needed) |
...
| `config/strategies/` | Genre-specific strategy configs (rpg, action, sports, puzzle, themepark, survival_horror, fighting) |
| `addresses/SLUS-00170.json` | Resident Evil (Director's Cut) memory address map |
| `addresses/SLUS-00402.json` | Tekken 3 memory address map |
...
python action_recommender.py recommend logs/20250101_120000_DEMO_agent.csv --log-dir logs/
python action_recommender.py recommend logs/20250101_120000_DEMO_agent.csv --log-dir logs/ --format json --top 5
...
# Session groups
python session_group.py --log-dir logs/ create "Experiment A" --description "Strategy comparison"
python session_group.py --log-dir logs/ add "Experiment A" 20250101_120000_DEMO_agent.csv
python session_group.py --log-dir logs/ remove "Experiment A" 20250101_120000_DEMO_agent.csv
python session_group.py --log-dir logs/ list
python session_group.py --log-dir logs/ show "Experiment A"
python session_group.py --log-dir logs/ show "Experiment A" --format json
python session_group.py --log-dir logs/ compare "Experiment A" "Experiment B"
python session_group.py --log-dir logs/ compare "Experiment A" "Experiment B" --format json
python session_group.py --log-dir logs/ delete "Experiment A"

# Session benchmarks
python session_benchmark.py --log-dir logs/ create DEMO --min-steps 50 --min-score 30 --param "hp >= 50" --param "gold > 1000" --description "Demo baseline"
python session_benchmark.py --log-dir logs/ list
python session_benchmark.py --log-dir logs/ show DEMO
python session_benchmark.py --log-dir logs/ evaluate logs/20250101_120000_DEMO_agent.csv
python session_benchmark.py --log-dir logs/ evaluate logs/20250101_120000_DEMO_agent.csv --format json
python session_benchmark.py --log-dir logs/ evaluate-all --game DEMO
python session_benchmark.py --log-dir logs/ delete DEMO

# A/B test
python session_ab_test.py --log-dir logs/ run "Group A" "Group B"
python session_ab_test.py --log-dir logs/ run "Group A" "Group B" --format json
python session_ab_test.py --log-dir logs/ run "Group A" "Group B" --param hp --alpha 0.01

# Session goals
python session_goal.py create survive --game DEMO --min-steps 10 --param "hp last >= 50" --description "Stay alive"
python session_goal.py delete survive --log-dir logs/
python session_goal.py list --log-dir logs/ --game DEMO
python session_goal.py evaluate 20250101_120000_DEMO_agent.csv --log-dir logs/ --format json
python session_goal.py evaluate-all --log-dir logs/ --game DEMO
python session_goal.py progress --log-dir logs/ --game DEMO --format json

# Replay annotations
python replay_annotator.py add 20250101_120000_DEMO_agent.csv 5 "Boss encounter" --category milestone
python replay_annotator.py remove 20250101_120000_DEMO_agent.csv <annotation_id>
python replay_annotator.py list --log-dir logs/
python replay_annotator.py show 20250101_120000_DEMO_agent.csv --format json

# Session lifecycle
python session_lifecycle.py evaluate --log-dir logs/
python session_lifecycle.py evaluate --log-dir logs/ --format json
python session_lifecycle.py evaluate --log-dir logs/ --policy policy.json
python session_lifecycle.py archive --log-dir logs/ --dry-run
python session_lifecycle.py archive --log-dir logs/
python session_lifecycle.py usage --log-dir logs/
python session_lifecycle.py usage --log-dir logs/ --format json
...
- **Adaptive strategy**: `AdaptiveStrategyEngine` switches strategy based on parameter thresholds. Multi-genre support via `config/strategies/` presets (rpg/action/sports/puzzle/themepark/survival_horror/fighting). `--strategy-config` CLI flag or `from_genre()` / `from_json()` classmethods. Priority-ordered evaluation
...
- **Action recommender**: `action_recommender.py` — `ActionRecommender` class takes training `list[SessionData]` + `top_n`. `_build_model()` aggregates action→parameter deltas across all training sessions into `impact_model` dict (action → param → mean_delta/count). `_compute_param_weights(session)` assigns urgency weights: declining params get +1.0, below-mean params get +0.5 (base weight 1.0). `recommend(target_session)` scores each action by sum of (mean_delta × param_weight), returns sorted `ActionScore` list. `to_dict()` / `to_markdown()` output. CLI: `python action_recommender.py recommend <csv> --log-dir logs/ [--top N] [--format json/markdown]`. Dashboard: `/session/{name}/recommend` page with urgency table + scored recommendation cards, `/api/session/{name}/recommend` JSON endpoint, Recommend button on session detail page
- **Session timeline chart**: `_timeline_chart_to_base64(session)` renders an integrated chart on the session detail page (`/session/{name}`). Primary y-axis: parameter lines (same color palette as comparison charts). Secondary y-axis: action colour bars via `ax.twinx()` + `barh()` using `Set3` colormap. Strategy switch events shown as purple dashed vertical lines from `session_info["strategy_switches"]`. `/api/session/{name}/timeline` JSON endpoint returns per-step action + parameter values, action counts, and strategy switches
...
- **Session groups**: `session_group.py` — `SessionGroupManager` class organises sessions into named experiment groups. Persists in `{log_dir}/session_groups.json`. `SessionGroup` dataclass (name/description/members). CRUD: `create_group()`, `delete_group()`, `get_group()`, `list_groups()`, `add_sessions()`, `remove_sessions()`, `rename_group()`. `group_stats()` computes aggregate stats via `SessionScorer`: avg score/steps/cost, per-parameter comparison (mean/min/max/std across members), member details. **Group comparison**: `compare_groups(group_a, group_b)` returns per-parameter mean diff, score diff, t-test significance (Welch's t-test via `scipy.stats.ttest_ind`), NaN-safe. `compare_markdown()` for formatted report. `plot_group_bar_comparison()` side-by-side bar chart, `plot_group_boxplot()` per-parameter box plots. `to_dict()` / `to_markdown()` output. CLI: `python session_group.py create|delete|add|remove|list|show|compare [--log-dir] [--format json/markdown]`. Dashboard: `/groups` page with group list + create form + group detail (stats table + parameter comparison + member list + add/remove forms), `/groups/compare` page with group selector + t-test table + bar chart + box plots, `/api/groups` GET (list) + POST (create/add/remove/delete), `/api/groups/compare` GET (comparison JSON), `/api/groups/{name}` GET (stats)
- **Session benchmarks**: `session_benchmark.py` — `SessionBenchmarkManager` class defines per-game benchmark criteria. Persists in `{log_dir}/session_benchmarks.json`. `BenchmarkCriteria` dataclass (game_id/min_steps/min_score/param_thresholds/description). `ParamThreshold` dataclass with aggregator (last/first/mean/min/max) + operator (>=/>/<=/</==/!=) + value. `ParamThreshold.parse("hp >= 50")` or `ParamThreshold.parse("gold mean > 1000")` for string parsing. CRUD: `create_benchmark()`, `delete_benchmark()`, `get_benchmark()`, `list_benchmarks()`. `evaluate(session)` returns `BenchmarkResult` with status (pass/fail/partial), per-criterion details (criterion/actual/passed). `evaluate_all(game_id)` batch evaluation. `to_dict()` / `to_markdown()` / `results_to_markdown()` output. CLI: `python session_benchmark.py create|delete|list|show|evaluate|evaluate-all [--log-dir] [--format json/markdown]`. Dashboard: `/benchmarks` page with create form + benchmark list + game detail (criteria display + evaluation results table with pass/fail/partial colour coding), `/api/benchmarks` GET (list) + POST (create/delete), `/api/benchmarks/{game_id}` GET (benchmark + evaluation results)
- **A/B test framework**: `session_ab_test.py` — `ABTestRunner` class for formal statistical comparison of two session groups. Uses `SessionGroupManager` to load groups, `SessionScorer` for scoring, `SessionData` for parameter extraction. `run(group_a, group_b, param=None)` returns `ABTestResult` with `score_test` (StatTestResult) + `param_tests` (list[StatTestResult]). `StatTestResult` dataclass per metric: mean_a/mean_b/diff, Welch's t-test (scipy.stats.ttest_ind, equal_var=False), Mann-Whitney U (scipy.stats.mannwhitneyu), Cohen's d (pooled SD), effect size label (negligible/small/medium/large), Welch-Satterthwaite confidence intervals, power analysis via non-central t distribution (scipy.stats.nct, with ncp>10 guard for numerical stability). `run_stat_test(metric, values_a, values_b, alpha)` runs full statistical battery. `result_to_markdown()` for publication-quality report. `to_dict()` for JSON output. CLI: `python session_ab_test.py run [--log-dir] [--alpha] [--param] [--format markdown/json]`. Dashboard: `/ab-test` page with group selector + alpha/param config + score/parameter comparison tables + effect size/CI/power, `/api/ab-test` JSON endpoint
- **Dashboard overview**: `dashboard.py` `_collect_overview()` + `_render_overview_cards()` — aggregates stats from SessionScorer (avg score all/recent 10), session_info strategy counts (top strategy), AnomalyDetector (anomaly count), SessionBenchmarkManager (benchmark pass rate), SessionLifecycleManager (disk usage MB). SVG sparkline polyline for recent 20 session scores. Score trend arrow (up/down/flat) comparing recent 10 avg vs all avg. All stats collected before tag/search filtering. `/api/overview` GET returns JSON dict. Cards rendered as inline flex blocks with card_style. Errors in optional modules (anomaly/benchmark/lifecycle) are caught and default to 0
- **Session goals**: `session_goal.py` — `SessionGoalManager` class for per-game objective tracking. Goals stored in `{log_dir}/session_goals.json` keyed by name. `Goal` dataclass (name/game_id/param_conditions/min_steps/description). Param conditions reuse `ParamThreshold.parse()` from session_benchmark.py (supports aggregators: last/first/mean/min/max). `create_goal()` parses condition strings → `ParamThreshold.to_dict()` for storage. `evaluate(session, goal_name=None)` returns `list[GoalResult]` (achieved bool + per-criterion details). `evaluate_all(game_id=None)` batch. `progress_over_time(game_id=None)` returns per-goal achievement timeline (sorted by timestamp). Dashboard: `/goals` page with CRUD form + evaluation table + achievement progress bars. `/api/goals` GET (+ `?game=` filter with evaluation+progress) + POST (create/delete)
- **Replay annotations**: `replay_annotator.py` — `ReplayAnnotator` class manages per-step annotations on session replay timelines. Annotations stored in `{log_dir}/session_annotations.json` keyed by CSV filename. `Annotation` dataclass (id/csv_filename/step/text/category/created_at) with UUID hex IDs. CRUD: `add_annotation()`, `remove_annotation()`, `get_annotations()` (sorted by step), `get_step_annotations()`, `list_sessions()`, `get_annotation_by_id()`. Categories: info/warning/success/danger/milestone. Empty text raises ValueError. Removing last annotation cleans session key. `to_dict()` / `to_markdown()` output. CLI: `python replay_annotator.py add|remove|list|show`. Dashboard: `/session/{name}/annotations` page with timeline bar (category-colored markers), annotation table with remove buttons, add form. `/api/session/{name}/annotations` GET (JSON) + POST (add/remove, JSON or form). Annotate button on session detail page
- **Session lifecycle**: `session_lifecycle.py` — `SessionLifecycleManager` class for session retention management. Configurable `RetentionPolicy` (keep_latest N, keep_tagged, keep_benchmark_pass, keep_tags whitelist). `evaluate_retention()` returns `list[RetentionDecision]` with keep/archive action + reasons per session. `archive(dry_run=False)` moves archivable sessions (CSV + .session.json + .history.json) to `{log_dir}/archive/`. `disk_usage()` returns `DiskUsageReport` with byte counts by file type + session/archived counts. Policy loaded from JSON via `RetentionPolicy.from_json()`. `to_dict()` / `to_markdown()` for report output. CLI: `python session_lifecycle.py evaluate|archive|usage [--log-dir] [--policy] [--format markdown/json] [--dry-run]`. Dashboard: `/lifecycle` page with policy config form + keep/archive decisions table + disk usage + dry-run archive + confirm archive, `/api/lifecycle` GET + `/api/lifecycle/archive` POST
